...
describe("Testando o método updateClientDataById da ClientBusiness", () => {
  const clientBusiness = new ClientBusiness(
    new ClientDatabaseMock(),
    new AuthenticatorMock()
  )

  test("Uma mensagem de sucesso é retornada quando o updateClientDataById é bem-sucedido", async () => {
    ...
    const input: any = {
      ...
      phone: "45632594874",
      adress: "Av. 7 de Setembro, 44",
      cpf: "55566688899"
    }

    const response = await clientBusiness.updateClientDataById(input, token)
  ...
  test("Erro quando algum dos parâmetros estiverem faltando", async () => {
    ...
    try {
      ...
      await clientBusiness.updateClientDataById(input, token)
  ...
  test("Erro quando 'name' ter menos que 4 e mais que 25 caracters", async () => {
    expect.assertions(2)
  ...
  test("Erro quando 'cpf' ter número de caracters diferente de 11", async () => {
    expect.assertions(2)
  ...
  test("Erro quando 'adress' ter mais que 45 caracters", async () => {
    expect.assertions(2)
    ...
    try {
      ...
      const input: any = {
        ...
        email: "teste@gmail.com",
        phone: "45632594874",
        adress: "Av. 7 de Setembrooooooooooooooooooooooooooooooo, 44",
        cpf: "55566688899"
      }
      ...
      if (error instanceof BaseError) {
        expect(error.statusCode).toBe(400)
        expect(error.message).toBe("Parâmetro 'Endereço' inválido: não pode haver mais de 45 caracters.")
      }
    }
  })
